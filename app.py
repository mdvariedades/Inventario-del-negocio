...
# Inicializar datos en la memoria si no existen
if 'inventario' not in st.session_state:
    st.session_state['inventario'] = pd.DataFrame(columns=["Producto", "Cantidad", "Precio Venta ($)"])
...
if menu == "Inventario":
    ...
    with st.form("form_producto"):
        ...
        if submit and nombre:
            nuevo_prod = pd.DataFrame([[nombre, cantidad, precio]], columns=["Producto", "Cantidad", "Precio Venta ($)"])
